...
describe("ZKCreditImport", function () {
    let vouchPayCore: VouchPayCore;
    let creditEngine: CreditEngine;
    let zkCreditImport: ZKCreditImport;
    ...
    beforeEach(async function () {
        ...
        vouchPayCore = await VouchPayCore.deploy(mockUSDC.target);

        // Deploy CreditEngine
        const CreditEngine = await ethers.getContractFactory("CreditEngine");
        creditEngine = await CreditEngine.deploy(vouchPayCore.target, mockUSDC.target);

        // Deploy ZKCreditImport
        const ZKCreditImport = await ethers.getContractFactory("ZKCreditImport");
        zkCreditImport = await ZKCreditImport.deploy(
            vouchPayCore.target,
            creditEngine.target,
            zkVerificationKey
        );
